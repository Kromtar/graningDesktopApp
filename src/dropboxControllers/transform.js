...
class TransformStream extends Transform {
	...
	_transform(chunk, encoding, next) {
		...
		// Chunk is big enough
		console.log(`Passing buffer (size: ${Math.round(this.buffer.byteLength / 1024)} KB)`);
	...
	_flush(next) {
		console.log('Flush the last chunk, that might be smaller than options.chunkSize');
		console.log(`Passing buffer (size: ${Math.round(this.buffer.byteLength / 1024)} KB)`);
		next(null, this.buffer);
		this.buffer = undefined;
	}
